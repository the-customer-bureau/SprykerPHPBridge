...
function echoJson(array $json): void{
    echo json_encode($json, JSON_THROW_ON_ERROR | JSON_PRETTY_PRINT);
}

try {
    echo ($bearerToken);
    echoJson($categoryTress);
    echoJson($category);
    echoJson($abstractProduct);
    echoJson($concreteProduct);
    echoJson($relatedProducts);
    echoJson($searchResults);
    echoJson($newWishList);
    echoJson($addToCartResponse);
    echoJson($postData);
    echoJson($checkout);
